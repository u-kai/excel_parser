...
fn main() {
    ...
    let root_node = XMLNode::from(source);
    let kai = root_node.search_child_by_id("id", "kai").unwrap();
    let children = kai.get_child_nodes().unwrap();
    println!("{:?}", children);
    let v = vec![""];
}
